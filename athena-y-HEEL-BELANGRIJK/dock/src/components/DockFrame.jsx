...
const DockFrame = () => {
  ...
  const handleReorderSections = async (newOrder) => {
    console.log("🔄 Reordering sections on server...");
    try {
        const dashboardPort = import.meta.env.VITE_DASHBOARD_PORT || '5001';
        const hostname = window.location.hostname;
  ...
  const handleSaveLayoutPreset = async (name) => {
    console.log(`💾 Saving current layout as preset: ${name}`);
    try {
        const dashboardPort = import.meta.env.VITE_DASHBOARD_PORT || '5001';
        const hostname = window.location.hostname;
        const res = await fetch(`http://${hostname}:${dashboardPort}/api/sites/${selectedSite.id}/save-layout-preset`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name })
        });
        if (res.ok) {
            alert(`✅ Layout '${name}' succesvol opgeslagen!`);
        }
    } catch (e) { console.error(e); }
  };
  ...
  const handleDisplayConfigSave = async (section, newConfig) => {
    console.log(`🧩 Saving display_config for ${section}:`, newConfig);
    try {
        const dashboardPort = import.meta.env.VITE_DASHBOARD_PORT || '5001';
        const hostname = window.location.hostname;
        const res = await fetch(`http://${hostname}:${dashboardPort}/api/sites/${selectedSite.id}/display-config`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
  ...
  const handleToggleSectionVisibility = async (section, isVisible) => {
    console.log(`👁️ Toggle visibility for ${section}: ${!isVisible}`);
    try {
        const dashboardPort = import.meta.env.VITE_DASHBOARD_PORT || '5001';
        const hostname = window.location.hostname;
        const res = await fetch(`http://${hostname}:${dashboardPort}/api/sites/${selectedSite.id}/section-settings`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ section, settings: { hidden: isVisible } })
        });
        if (res.ok) {
            forceRefresh();
        }
  ...
  useEffect(() => {
    const handleMessage = (event) => {
      ...
      if (event.data.type === 'UPDATE_SECTION_SETTINGS') {
        const { section, settings } = event.data;
        fetch(`/api/sites/${selectedSite.id}/section-settings`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ section, settings })
        }).then(res => { if (res.ok) forceRefresh(); });
      }
  ...
  const handleEditorSave = async (newValue, newFormatting = null) => {
    ...
    // Handle Global Actions (like ADD_SECTION)
    if (newValue?.type === 'ADD_SECTION') {
        console.log("➕ Adding new section:", newValue.sectionId);
        try {
            const res = await fetch(`/api/sites/${selectedSite.id}/add-section`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ sectionId: newValue.sectionId })
            });
            if (res.ok) {
                // Refresh to see changes
                window.location.reload();
            } else {
                alert("Fout bij toevoegen sectie.");
            }
        } catch (e) {
            console.error(e);
        }
  ...
  const deleteSection = async (sectionName) => {
    ...
    try {
        const res = await fetch(`/api/sites/${selectedSite.id}/delete-section`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
  ...
  const handleAIRedesign = async (sectionName, goal) => {
    ...
    try {
        ...
        if (result.success) {
            ...
            // Apply settings
            const saveRes = await fetch(`/api/sites/${selectedSite.id}/section-settings`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
  ...
  const handleAIABTest = async (sectionName) => {
    ...
    try {
        const res = await fetch('/api/ai/layout/ab-test', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
                siteId: selectedSite.id, 
  ...
  const toggleSectionVisibility = (sectionId) => {
    ...
    const currentVisible = siteStructure?.data?.section_settings?.[sectionId]?.visible !== false;
    ...
    setSiteStructure(prev => {
      ...
      settings[sectionId] = { ...(settings[sectionId] || {}), visible: nextVisible };
      newData.section_settings = settings;
      return { ...prev, data: newData };
    });

    saveData('section_settings', sectionId, 'visible', nextVisible);
  };
    ...
    <div className="dock-container h-screen flex flex-col bg-slate-100">
      ...
      <header className="bg-slate-900 text-white p-4 flex items-center justify-between shadow-lg z-50">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={() => setShowNavigationManager(true)}
            className="px-3 py-1 bg-emerald-600 hover:bg-emerald-500 text-[10px] text-white rounded font-bold uppercase tracking-widest flex items-center gap-2 transition-all shadow-lg shadow-emerald-500/20"
            title="Open de Menu Builder om navigatie links te beheren."
          >
          ...
          <button
            ...
            className="px-3 py-1 bg-blue-600 hover:bg-blue-500 text-[10px] text-white rounded font-bold uppercase tracking-widest flex items-center gap-2 transition-all shadow-lg shadow-blue-500/20"
            title="Open de Layout Builder om secties te verplaatsen, verbergen of toe te voegen."
          >
      ...
      {showHelpModal && <HelpModal selectedSite={selectedSite} onClose={() => setShowHelpModal(false)} />}

      {showLayoutManager && (
        <LayoutManager 
            sectionOrder={siteStructure?.data?.section_order} 
            sectionSettings={siteStructure?.data?.section_settings}
            onReorder={handleReorderSections}
            onToggleVisibility={handleToggleSectionVisibility}
            onDelete={(name) => { if (window.confirm("Sectie verwijderen?")) deleteSection(name); }}
            onDuplicate={handleDuplicateSection}
            onAdd={() => { setEditingItem({ dockType: 'add_section' }); setShowLayoutManager(false); }}
            onApplyPreset={handleApplyLayoutPreset}
            onSaveAsPreset={handleSaveLayoutPreset}
      ...
      {showNavigationManager && (
        <NavigationManager 
            navigationData={siteStructure?.data?.navigation}
      ...
      {fieldConfigItem && (
        <FieldConfigModal 
            sectionName={fieldConfigItem.name} 
            currentConfig={fieldConfigItem.config}
            onSave={handleDisplayConfigSave}
      ...
      {sectionDesignItem && (
        <SectionDesignModal 
            sectionName={sectionDesignItem.name} 
            currentSettings={sectionDesignItem.settings}
            currentLayout={siteStructure?.data?.layout_settings?.[sectionDesignItem.name]}
            onSave={async (name, settings) => {
                const dashboardPort = import.meta.env.VITE_DASHBOARD_PORT || '5001';
                const hostname = window.location.hostname;
                await fetch(`http://${hostname}:${dashboardPort}/api/sites/${selectedSite.id}/section-settings`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ section: name, settings })
                });
                forceRefresh();
            }}
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex gap-1 items-center">
                    <button
                      ...
                      className={`p-1 rounded mr-1 ${siteStructure?.data?.section_settings?.[section]?.visible === false ? 'text-slate-300 bg-slate-100 hover:bg-slate-200' : 'text-blue-500 bg-blue-50 hover:bg-blue-100'}`}
                      title={siteStructure?.data?.section_settings?.[section]?.visible === false ? "Sectie is verborgen op de site. Klik om te tonen." : "Sectie is zichtbaar op de site. Klik om te verbergen."}
                    >
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="w-24">
                      ...
                      <input 
                        ...
                        value={siteStructure?.data?.section_settings?.[section]?.padding || 32}
                        onInput={(e) => {
                          ...
                          setSiteStructure(prev => {
                            ...
                            const newData = { ...prev.data };
                            const settings = { ...(newData.section_settings || {}) };
                            settings[section] = { ...(settings[section] || {}), padding: val };
                            newData.section_settings = settings;
                            return { ...prev, data: newData };
                          });
                        }}
                        onChange={(e) => {
                          const val = parseInt(e.target.value);
                          saveData('section_settings', section, 'padding', val);
                        }}
                      />
                    </div>
                  </div>
