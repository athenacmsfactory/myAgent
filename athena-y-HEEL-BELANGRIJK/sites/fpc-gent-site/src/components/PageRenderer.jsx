    ...
    useEffect(() => {
        const loadPage = async () => {
            ...
            try {
                ...
                // --- STRATEGY: Try Aggregated Data first ---
                let pageData = null;
                try {
                    const allDataRes = await fetch(base + 'src/data/all_data.json');
                    if (allDataRes.ok) {
                        const allData = await allDataRes.json();
                        if (allData._pages && allData._pages[pageKey]) {
                            console.log(`🚀 [PageRenderer] Using aggregated data for: ${pageKey}`);
                            pageData = allData._pages[pageKey];
                            if (allData.style_bindings) setStyles(allData.style_bindings);
                        }
                    }
                } catch (e) { /* Fallback to individual fetch */ }

                // --- FALLBACK: Individual Page Fetch ---
                if (!pageData) {
