...
async function init() {
  ...
  try {
    ...
    // --- STYLE CONFIG MERGE (Governance V7) ---
    const styleConfig = getData('style_config');
    if (styleConfig) {
        // Zorg dat we arrays en objecten correct behandelen
        const settingsArr = Array.isArray(data['site_settings']) ? data['site_settings'] : [data['site_settings']];
        const styleArr = Array.isArray(styleConfig) ? styleConfig : [styleConfig];
        
        const settingsObj = settingsArr[0] || {};
        const styleObj = styleArr[0] || {};
        
        // Merge styles in settings
        data['site_settings'] = [{ ...settingsObj, ...styleObj }];
        console.log("🎨 Styles merged from style_config.json");
    }
