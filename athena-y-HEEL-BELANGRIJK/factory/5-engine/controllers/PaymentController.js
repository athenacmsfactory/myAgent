...
export class PaymentController {
    ...
    async createStripeSession(projectName, cart, successUrl, cancelUrl) {
        // 1. Determine Mode
        const config = this.getProjectPaymentConfig(projectName);
        const isLive = config.test_mode === false;
        
        const stripeKey = isLive ? process.env.STRIPE_LIVE_SECRET_KEY : process.env.STRIPE_SECRET_KEY;
        
        if (!stripeKey || stripeKey.includes('sk_test_...')) {
            throw new Error(`Stripe ${isLive ? 'LIVE' : 'TEST'} key niet correct geconfigureerd in .env`);
        }

        console.log(`💳 [Payment] Creating ${isLive ? 'LIVE' : 'TEST'} Stripe session for ${projectName}...`);
