...
async function tryGoogle({ modelName, prompt, isJson, temp }) {
    ...
    // ✅ Gemini 3.0 Compliance: Robust parts extraction
    const candidates = result.response.candidates || [];
    const parts = candidates[0]?.content?.parts || [];
    
    // Optionally log thought for debugging
    const thoughtPart = parts.find(p => p.thought);
    if (thoughtPart && process.env.DEBUG_AI === 'true') {
        console.log("🧠 [AI-ENGINE] Gemini Thought:", thoughtPart.thought);
    }
