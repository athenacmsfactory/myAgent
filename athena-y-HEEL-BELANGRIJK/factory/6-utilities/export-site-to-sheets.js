...
try {
    ...
    // 3. Update Sync Timestamp in Site Settings
    console.log("\n🚀 Stap 3: Synchronisatie timestamp bijwerken...");
    const timestamp = new Date().toISOString();
    const siteSettingsPath = path.resolve(root, `../sites/${siteName}/src/data/site_settings.json`);
    if (fs.existsSync(siteSettingsPath)) {
        try {
            ...
            if (data[0]) {
                data[0].last_sheet_sync = timestamp;
                fs.writeFileSync(siteSettingsPath, JSON.stringify(data, null, 2));
                console.log(`✅ Timestamp opgeslagen: ${timestamp}`);
            }
        } catch (e) {
            console.warn("⚠️ Kon timestamp niet opslaan in site_settings.json");
        }
    }
