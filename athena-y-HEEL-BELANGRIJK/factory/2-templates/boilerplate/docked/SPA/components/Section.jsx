...
const Section = ({ data: initialData }) => {
  ...
  useEffect(() => {
    ...
    const loadTranslations = async () => {
        try {
            ...
            const res = await fetch(`${base}src/data/all_data_${currentLang}.json`);
            if (res.ok) {
                const translatedData = await res.json();
                setData(translatedData);
            }
        } catch (e) { console.error("❌ [i18n] Error loading translations:", e); }
    };
    loadTranslations();
  ...
  return (
    <div className="flex flex-col relative">
      {/* 🌍 Language Switcher */}
      <div className="absolute top-4 right-6 z-[100] no-print">
        <LanguageSwitcher 
      ...
      {sectionOrder.filter(name => {
        if (name === 'site_settings') return false;
        if (isDockActive) return true; // Show all in dock
        const settings = sectionSettings[name] || {};
        return settings.visible !== false && settings.hidden !== true;
      }).map((sectionName, idx) => {
        ...
        const sectionStyle = {
            backgroundColor: currentSettings.backgroundColor || null,
            paddingTop: currentSettings.paddingTop || null,
            paddingBottom: currentSettings.paddingBottom || null,
            borderRadius: currentSettings.borderRadius || null,
            textAlign: currentSettings.alignment || 'left'
        };
        const currentLayout = layoutSettings[sectionName] || 'list';
        const isVisible = currentSettings.visible !== false && currentSettings.hidden !== true;

        const handleToggleVisibility = () => {
            window.parent.postMessage({ 
                type: 'UPDATE_SECTION_SETTINGS', 
                section: sectionName, 
                settings: { ...currentSettings, hidden: isVisible } 
            }, '*');
        };
        ...
        if (sectionName === 'basis' || sectionName === 'hero') {
          ...
          return (
            <section
              ...
              className={`relative group w-full h-auto min-h-[var(--hero-height,85vh)] max-h-[var(--hero-max-height,150vh)] aspect-[var(--hero-aspect-ratio,16/9)] flex items-center justify-center overflow-hidden bg-[var(--color-hero-bg)] pt-24 ${!isVisible ? 'opacity-40 grayscale' : ''}`}
              style={sectionStyle}
            >
              {isDockActive && (
                <SectionToolbar 
        ...
        return (
          <section 
            ...
            id={sectionName} 
            data-dock-section={sectionName} 
            className={`relative group py-24 px-6 bg-[var(--color-background)] ${!isVisible ? 'opacity-40 grayscale' : ''}`} 
            style={sectionStyle}
          >
            {isDockActive && (
                <SectionToolbar 
