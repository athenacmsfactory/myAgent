...
export default function EditableMedia({ src: rawSrc, alt, className, cmsBind, dataItem = {}, ...props }) {
  ...
  return (
    ...
    >
      ...
      {/* EDITOR OVERLAY */}
      <div className="absolute inset-0 bg-blue-600/40 flex flex-col items-center justify-center transition-opacity duration-300 pointer-events-none" style={{ opacity: isHovering || isUploading ? 1 : 0, zIndex: 50 }}>
      ...
      <div className="absolute top-4 right-4 z-[60] flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
        {isVideo && src && (
            <button onClick={toggleLoop} className={`p-2 rounded-full border border-white/20 shadow-xl transition-all ${isLooping ? 'bg-blue-600 text-white' : 'bg-slate-800 text-white/50'}`} title={isLooping ? "Loop is AAN" : "Loop is UIT"}>
                <i className={`fa-solid ${isLooping ? 'fa-rotate' : 'fa-play'}`}></i>
            </button>
        )}
        {src && (
            <button onClick={handleReset} className="bg-red-600 text-white p-2 rounded-full hover:bg-red-700 shadow-xl border border-white/20">
