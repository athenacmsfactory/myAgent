...
let root = __dirname;
// We know we are in factory/6-utilities, so we need to go up 2 levels to get to athena-x
root = path.resolve(__dirname, '../..'); 

if (!fs.existsSync(path.join(root, 'pnpm-workspace.yaml'))) {
    // Fallback search if structure changes
    let search = __dirname;
    while (search !== '/' && !fs.existsSync(path.join(search, 'athena-x'))) {
        search = path.dirname(search);
    }
    if (fs.existsSync(path.join(search, 'athena-x'))) root = path.join(search, 'athena-x');
}
...
console.log(`\n✅ Audit voltooid. Rapport opgeslagen in athena-x/output/SITES_PORTFOLIO_AUDIT.md`);
