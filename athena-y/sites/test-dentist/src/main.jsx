...
async function init() {
  ...
  try {
    ...
    try { 
        const tableData = (await import('./data/behandelingen.json')).default;
        data.behandelingen = tableData;
        if (Array.isArray(tableData)) totalRows += tableData.length;
    } catch (e) { console.warn("Mist behandelingen.json"); }
    try { 
        const tableData = (await import('./data/team.json')).default;
        data.team = tableData;
        if (Array.isArray(tableData)) totalRows += tableData.length;
    } catch (e) { console.warn("Mist team.json"); }
