...
(function () {
    ...
    function notifyDock(fullData = null) {
        ...
        // Extract computed styles for the sidebar to stay in sync
        const computed = {};
        const rootStyle = getComputedStyle(document.documentElement);
        const isDark = document.documentElement.classList.contains('dark');
        const prefix = isDark ? 'dark_' : 'light_';

        Object.entries(themeMappings).forEach(([key, vars]) => {
            // Try each variable in the mapping until we find one that has a value
            for (const v of vars) {
                const val = rootStyle.getPropertyValue(v).trim();
                if (val) {
                    computed[`${prefix}${key}`] = val;
                    break;
                }
            }
        });
        ...
        const structure = {
            ...
            data: { ...(lastKnownData || {}), ...computed }, // Merge computed styles into data
